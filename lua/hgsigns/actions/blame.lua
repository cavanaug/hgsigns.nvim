...
local BlameFormatter = require('hgsigns.blame_formatter')
...
local get_temp_hl = require('hgsigns.highlight').get_temp_hl
...
--- @param blame Hgsigns.CacheEntry.Blame
--- @param win integer
...
local function render(blame, win, main_win, buf_sha)
  ...
  local highlights = {} --- @type table<integer, Hgsigns.BlameLineHighlight[]>
  ...
  for i, b in ipairs(entries) do
    ...
    local line_chunks --- @type Hgsigns.BlameFmtChunk[]
    ...
    if sha == last_sha then
      ...
      line_chunks = { { graph, hash_hl } } --- @type Hgsigns.BlameFmtChunk[]
...
local function reblame(opts, blame, win, revision, parent)
  ...
  local did_attach = require('hgsigns.actions.diffthis').show(nil, sha)
...
local function show_commit(win, bwin, open, bcache)
  ...
  api.nvim_set_current_win(win)
  require('hgsigns.actions.show_commit')(sha, open)
end
...
local function diff(bufnr, blm_win, blame)
  ...
  require('hgsigns.actions.diffthis').show(bufnr, info.commit.sha, info.filename)
  if info.previous_sha then
    require('hgsigns.actions').diffthis(info.previous_sha)
  end
end
...
--- @param blm_win integer
--- @param entries table<integer,Hgsigns.BlameInfo?>
--- @param min_time integer
--- @param max_time integer
