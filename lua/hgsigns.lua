...
--- @class hgsigns.main: hgsigns.actions,hgsigns.attach,hgsigns.debug
local M = {}
...
local function config()
  return require('hgsigns.config').config
end

local function async()
  return require('hgsigns.async')
end
...
local function get_gitdir_and_head()
  ...
  if package.loaded['hgsigns.cache'] then
    for _, bcache in pairs(require('hgsigns.cache').cache) do
  ...
  local info = require('hgsigns.git').Repo.get_info(cwd)
...
local function setup_cli()
  api.nvim_create_user_command('Hgsigns', function(params)
    ...
    complete = function(arglead, line)
      return require('hgsigns.cli').complete(arglead, line)
    end,
  })
end
...
function M.setup(cfg)
  ...
  if cfg then
    require('hgsigns.config').build(cfg)
  end
...
function M.statuscolumn(bufnr, lnum)
  return require('hgsigns.manager').statuscolumn(bufnr, lnum)
end
...
M = setmetatable(M, {
  __index = function(_, f)
    ...
    if config().debug_mode then
      local debug = require('hgsigns.debug')
      if debug[f] then
        return debug[f]
      end
    end
