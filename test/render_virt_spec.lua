...
describe('render.virt', function()
  ...
  it('renders layered captured lines and extends eol fill with padding', function()
    ...
    eq(24, width)
    assert(contains_hl(virt_hl_at_col(vline, 0), 'HgsignsDeleteVirtLn'))
    assert(contains_hl(virt_hl_at_col(vline, 7), 'HgsignsDeleteVirtLnInLine'))
    assert(contains_hl(virt_hl_at_col(vline, 20), 'HgsignsDeleteVirtLn'))
  end)
      ...
      local lines = {
        {
          text = 'alpha',
          layers = {
            { start_col = 0, end_col = 5, priority = 1000, hl_group = 'HgsignsDeleteVirtLn' },
          },
        },
        {
          text = 'beta',
          layers = {
            { start_col = 0, end_col = 4, priority = 1000, hl_group = 'HgsignsDeleteVirtLn' },
          },
        },
      }
