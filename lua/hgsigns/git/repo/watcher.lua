...
--- Polling tracks logical metadata targets. The actual poll handle may
...
local function should_notify_fs_event(filename, events, vcs)
  ...
  log.dprintf("Hg dir update: '%s' %s", filename, details)
  return true
end

--- Poll callbacks only tell us that a watched handle path changed. Compare
--- logical target fingerprints so we only notify on relevant dir changes.
...
--- unrelated parent-dir changes and only notify on metadata updates.
