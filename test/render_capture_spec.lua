...
describe('render capture', function()
  ...
  it('applies full-line and word-diff overlays without rendering', function()
    local full_stack, diff_stack, plain_stack = exec_lua(function()
      ...
      Overlay.add_full_line_layer(lines, 'HgsignsDeleteVirtLn', 1000)
      Overlay.add_word_diff_layers(lines, { { 1, 'change', 2, 3 } }, function(region_type)
        return region_type == 'change' and 'HgsignsChangeInline' or nil
    ...
    eq({ 'HgsignsDeleteVirtLn' }, full_stack)
    eq({ 'HgsignsDeleteVirtLn', 'HgsignsChangeInline' }, diff_stack)
    eq({ 'HgsignsDeleteVirtLn' }, plain_stack)
  end)
end)
