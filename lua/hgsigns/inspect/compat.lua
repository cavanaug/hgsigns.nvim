...
--- @class Hgsigns.InspectCompatFilter
...
--- @class Hgsigns.InspectCompatCapture
...
--- @return Hgsigns.HlName|Hgsigns.HlStack|nil
--- @return string?
local function resolve_hl_group(hl)
  ...
  if type(hl) == 'table' then
    local groups = {} --- @type Hgsigns.HlStack
...
--- @return Hgsigns.InspectPosExtmarkItem
local function extmark_to_map(extmark, nsmap)
  ...
  } --- @type Hgsigns.InspectPosExtmarkItem

  return resolve_hl(item) --[[@as Hgsigns.InspectPosExtmarkItem]]
end
...
--- @param row integer
--- @param cols integer[]
--- @return Hgsigns.InspectPosTSItem[]
local function collect_treesitter(bufnr, row, cols)
  local items = {} --- @type Hgsigns.InspectPosTSItem[]
  local previous = {} --- @type table<string, Hgsigns.InspectPosTSItem>

  for i = 1, #cols - 1 do
    local col = assert(cols[i])
    local next_col = assert(cols[i + 1])
    local active = {} --- @type table<string, Hgsigns.InspectPosTSItem>

    for _, capture in pairs(vim.treesitter.get_captures_at_pos(bufnr, row, col)) do
      --- @cast capture Hgsigns.InspectCompatCapture
...
--- @param row integer
--- @param col integer
--- @param opts? Hgsigns.InspectCompatFilter
--- @return Hgsigns.InspectPosResult
function M.inspect_pos(bufnr, row, col, opts)
  opts = vim.tbl_deep_extend('force', defaults, opts or {}) --[[@as Hgsigns.InspectCompatFilter]]

  local has_range = opts.end_row ~= nil and opts.end_col ~= nil
  if has_range and opts.end_row ~= row then
    error('hgsigns inspect backend only supports single-row ranges')
  end
  ...
  --- @type Hgsigns.InspectPosResult
  local result = {
    treesitter = {}, --- @type Hgsigns.InspectPosTSItem[]
    syntax = {}, --- @type Hgsigns.InspectPosItem[]
    extmarks = {}, --- @type Hgsigns.InspectPosExtmarkItem[]
    semantic_tokens = {}, --- @type Hgsigns.InspectPosExtmarkItem[]
    buffer = bufnr,
    row = row,
    col = col,
  }
