...
local Virt = require('hgsigns.render.virt')
...
--- @return Hgsigns.VirtTextChunk[]
local function chunks_from_statusline(str, highlights)
  local chunks = {} --- @type Hgsigns.VirtTextChunk[]
...
--- @param win integer
--- @param lnum integer
--- @param opts? {extra_hl?: Hgsigns.HlName|Hgsigns.HlStack}
--- @return Hgsigns.VirtTextChunk[]
...
local function get_win_ns(winid, bufnr)
  ...
  if not entry then
    entry = {
      ns = api.nvim_create_namespace(('hgsigns_removed_win_%d'):format(winid)),
    }
    win_ns[winid] = entry
...
local function render_virt_lines(lines, start_lnum, opts)
  ...
  local prefix --- @type fun(line_index: integer, line: Hgsigns.CapturedLine): Hgsigns.VirtTextChunk[]?
  if opts.lno_hl then
...
--- @param captured Hgsigns.CapturedLine[][]
...
--- @param source_bufs table<string, Hgsigns.HunkPreview.SourceBuf>?
...
--- @param bufnr integer
--- @param state {hunks: Hgsigns.Hunk.Hunk[], lines: table<Hgsigns.Hunk.Hunk, Hgsigns.CapturedLine[]>, source_bufs: table<string, Hgsigns.HunkPreview.SourceBuf>, pending?: table<Hgsigns.Hunk.Hunk, true>, scheduled?: boolean}
local function flush_pending_entries(bufnr, state)
  ...
  local missing = {} --- @type Hgsigns.Hunk.Hunk[]
...
function M.on_win(winid, bufnr, topline, botline)
  ...
  local visible = {} --- @type {row: integer, above: boolean, start_lnum: integer, lines: Hgsigns.CapturedLine[]}[]
  local missing = {} --- @type Hgsigns.Hunk.Hunk[]
  for _, hunk in ipairs(state.hunks) do
    local row, above = show_deleted_placement(hunk)
