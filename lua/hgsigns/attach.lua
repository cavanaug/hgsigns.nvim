...
local throttle_async = require('hgsigns.debounce').throttle_async

local api = vim.api
local current_buf = api.nvim_get_current_buf
local uv = vim.uv or vim.loop ---@diagnostic disable-line: deprecated

--- @class hgsigns.attach
local M = {}
...
--- @class (exact) Hgsigns.GitContext
...
local function handle_moved(bufnr, old_relpath)
  ...
  local new_name = git_obj.repo:diff_rename_status(git_obj.repo.head_oid)[orig_relpath]
    or git_obj.repo:diff_rename_status()[orig_relpath]
  if new_name then
...
local function repo_update_handler(bufnr)
  ...
  require('hgsigns.manager').update(bufnr)
end

--- @param opts Hgsigns.AttachOpts?
...
M.attach = throttle_async({ hash = attach_hash }, function(opts)
  ...
  if config.current_line_blame then
    require('hgsigns.current_line_blame').update(cbuf)
  end
end)
